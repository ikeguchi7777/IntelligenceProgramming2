...
public class Search {
	...
	Search(int which_change) {
		makeStateSpace(which_change);
	}

	private void makeStateSpace(int which_change) {
		node = new Node[10];
		// 状態空間の生成
		...
		else node[5] = new Node("Sandiego", 2);
		...
		if(which_change == 2){
			node[1].addChild(node[6], 11);
			node[2].addChild(node[3], 10);
			node[2].addChild(node[6], 10);
		}
		else{
	...
	public static void main(String[] args) {
		if (args.length != 2) {
			...
			int which_change = Integer.parseInt(args[1]);
			switch (which) {
			case 1:
				// 幅優先探索
				System.out.println("\nBreadth First Search");
				(new Search(which_change)).breadthFirst();
				break;
			case 2:
				// 深さ優先探索
				System.out.println("\nDepth First Search");
				(new Search(which_change)).depthFirst();
				break;
			case 3:
				// 分岐限定法
				System.out.println("\nBranch and Bound Search");
				long a = System.nanoTime();
				(new Search(which_change)).branchAndBound();
			...
			case 4:
				...
				(new Search(which_change)).hillClimbing();
				break;
			case 5:
				// 最良優先探索
				System.out.println("\nBest First Search");
				(new Search(which_change)).bestFirst();
				break;
			case 6:
				// A*アルゴリズム
				System.out.println("\nA star Algorithm");
				(new Search(which_change)).aStar();
				break;
			default:
